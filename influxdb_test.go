...
func TestInfluxDB(t *testing.T) {
	...
	influxDBRegistry := RegisterInfluxDB(context.Background(), &InfluxDB{Address: ":8089", PushInterval: Duration(time.Second), AddEntryPointsLabels: true, AddServicesLabels: true})
	defer StopInfluxDB()
...
func TestInfluxDBHTTP(t *testing.T) {
	...
	influxDBRegistry := RegisterInfluxDB(context.Background(), &InfluxDB{Address: ts.URL, Protocol: "http", PushInterval: Duration(time.Second), Database: "test", RetentionPolicy: "autogen", AddEntryPointsLabels: true, AddServicesLabels: true})
	defer StopInfluxDB()
